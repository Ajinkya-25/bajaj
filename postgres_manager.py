...
class PostgresManager:
    ...
    def store_document_with_url(self, filename: str, document_url: str, file_type: str, file_size: int, categories: List[str], processing_time: float) -> int:
        """Store document with URL and additional metadata"""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        INSERT INTO documents (filename, document_url, file_type, file_size, categories, processing_time)
                        VALUES (%s, %s, %s, %s, %s, %s)
                        RETURNING id
                    """, (filename, document_url, file_type, int(file_size), json.dumps(categories), float(processing_time)))
                    document_id = cur.fetchone()[0]
                    conn.commit()
            logger.info("Stored document '%s' from URL with id=%s", filename, document_id)
            return document_id
        except Exception as e:
            logger.error("Failed to store document with URL '%s': %s", filename, e, exc_info=True)
            raise
    ...
    def store_enhanced_chunks(self, document_id: int, chunks: List[Dict[str, Any]]):
        """Store enhanced document chunks with categories and importance scores"""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    for i, chunk in enumerate(chunks):
                    ...
                    conn.commit()
            logger.info("Stored %d enhanced chunks for document id=%s", len(chunks), document_id)
        except Exception as e:
            logger.error("Failed to store enhanced chunks for document id=%s: %s", document_id, e, exc_info=True)
            raise
        ...
        try:
            ...
            with self.get_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    """, (clean_vector_ids,))
                    rows = [dict(row) for row in cur.fetchall()]
            logger.info("Retrieved %d chunks with categories for vector IDs %s", len(rows), clean_vector_ids)
            return rows
        except Exception as e:
            logger.error("Failed to retrieve chunks with categories for vector IDs %s: %s", vector_ids, e, exc_info=True)
            raise
